...
const NormalSale: React.FC<ReceiptProps> = ({ receiptData }) => {
  const {
    tradeName,
    address,
    tin,
  ...
  return (
    ...
    >
      ...
      <div style={{ fontSize: '10px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '10px' }}>
         ...
         <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '10px' }}>
          ...
          <span>{receiptNumber}/{receiptNumber} NS</span>
      ...
      <div style={{ display: 'flex', justifyContent: 'center', margin: '2mm 0' }}>
        <QRCode 
          value={`${DateUtils.parse(date).toLocaleDateString()}#${DateUtils.parse(time).toLocaleTimeString()}#${sdcId}#${receiptNumber}#${internalData?.padEnd(40,'')?.match(/.{1,4}/g)?.join('-')}#${receiptSignature?.padEnd(20,'')?.match(/.{1,4}/g)?.join('-')}`}  
          size={80} 
        />
      </div>
