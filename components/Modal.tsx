...
export default function Modal({
  ...
  return (
    <>
      {/* Background overlay */}
      <div
        className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[49] modal-overlay transition-opacity duration-200"
      ...
      <div className="fixed top-4 left-0 right-0 bottom-0 flex items-start justify-center z-[1000] overflow-y-auto p-4">
        <div
          className={`relative rounded-lg bg-white shadow-xl w-full ${sizeClasses[size]} max-h-[calc(100vh-2rem)]`}
        ...
        >
          ...
          {/* Modal content */}
          <div className="px-6 py-4 overflow-y-auto max-h-[calc(100vh-7rem)]">
            {children}
          </div>
        </div>
      </div>
