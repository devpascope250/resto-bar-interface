...
export default function InventoryReportPage() {
  ...
  const {data: inventoryReport } = useApiQuery<MonthlyRemport[]>(['inventory-report'], '/bar/products/report');
  const [filterCategory, setFilterCategory] = useState<"all" | "BEVERAGE" | "FOOD">("all")
  ...
  const filteredData = useMemo(() => {
    ...
    return inventoryReport?.filter(
      (item) => item.productType === (filterCategory === "BEVERAGE" ? "BEVERAGE" : "FOOD"),
    )
  }, [inventoryReport, filterCategory])

  const inventoryStats = useMemo(() => {

    const totalStockIn = inventoryReport?.filter((t) => t.totalIn).reduce((sum, t) => sum + t.totalIn, 0)
    const totalStockOut = inventoryReport?.filter((t) => t.totalOut).reduce((sum, t) => sum + t.totalOut, 0)
    ...
    <div className="p-8">
      ...
      <div className="mb-6 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-green-600">{inventoryStats.totalStockIn ?? 0}</div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-red-600">{inventoryStats.totalStockOut ?? 0}</div>
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <Select
              ...
              onValueChange={(value: "all" | "BEVERAGE" | "FOOD") => setFilterCategory(value)}
            >
